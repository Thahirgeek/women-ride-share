      ...
      <aside className="hidden w-64 shrink-0 border-r border-(--border) bg-white lg:block">
        <div className="flex h-full flex-col">
          ...
          <div className="border-t border-(--border) px-4 py-4">
            {userName && (
              <p className="mb-2 truncate px-3 text-xs text-(--text-3)">
                {userName}
              </p>
            )}
            <button
              onClick={handleSignOut}
              className="flex w-full cursor-pointer items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-(--text-2) transition-all duration-200 hover:bg-(--bg-muted) hover:text-foreground"
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
